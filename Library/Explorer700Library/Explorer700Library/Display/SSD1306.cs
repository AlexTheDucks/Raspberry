...
namespace Explorer700Library
{
    public class SSD1306 : IDisposable
    {
        ...
        {
            /*
            var resourceNames = typeof(SSD1306).Assembly.GetManifestResourceNames();
            var fontResource = resourceNames.First(r => r.Contains("AsciiFontPng"));            // Can not find this file!
            using var stream = typeof(SSD1306).Assembly.GetManifestResourceStream(fontResource);
        ...
        public SSD1306(DisplayModel model)
        ...
        public SSD1306(II2CDevice device, DisplayModel model, VccSourceMode vccSource)
        {
            switch (model)
            {
        ...
        public void Render()
        {
            //SendCommand(Command.SetColumnAddressRange, 0, (byte)(Width - 1));             // Moved to SetAddressRange()
            //SendCommand(Command.SetPageAddressRange, 0, (byte)(_bufferPageCount - 1));    // Moved to SetAddressRange()
        ...
        /// </summary>
        /// <param name="command">The command.</param>
        //private void SendCommand(byte command) => Device.Write(new byte[] { 0x01, command });
        private void SendCommand(byte command) => Device.Write(new byte[] { command });
